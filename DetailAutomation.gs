...
//Function: onEdit
//Description: Runs every time the spreadsheet is changed and checks to see if a resident signed off on a detail.
...
function onEdit() {
  ...
  //Retrieve information about the active row
  ...
  if(activeCell.isBlank() == false) {
    ...
    var managerEmail = "hm.clo1931@gmail.com"
...
//Function: confirmDetailsComplete
//Description: Runs every time the spreadsheet is changed and checks to see if a resident signed off on a detail.
  //If it is confirmed that a resident signed off, sends an email to the manager notifying them of detail completion.
//Requirements: must include a trigger in order for emails to be sent properly.
  //To setup a trigger
...
function confirmDetailsComplete() {
  ...
  for(i in data) {
    ...
    if(fined == false) {
      ...
      var message = "Detail ___ completed on ____ and such"
      
          } else {
            var subject = "Detail not completed"
            var message = "Detail ___ not completed on ____ and such"
          }
    ...
    sheet.getRange(startRow, 1, numRows, 9).copyTo(targetSheet);
  }
